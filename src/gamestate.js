...
export function newGame() {
  var game = {
    ...
    hero: {
      pos: {
        x: 20 * scaling.SCALE_X,
        y: 20 * scaling.SCALE_Y,
      },
      size: {
...
export function newPipes(game) {
  ...
  do {
    ...
    game.lastHole &&
    Math.abs(hole - game.lastHole) < holeSize.height * 0.5 &&
    iterations < 5
  );
...
export function checkScore(game) {
  ...
  for (var i = 0; i < game.pipes.length; i += 2) {
    ...
    if (
      topPipe &&
      !topPipe.passed &&
      heroRight > topPipe.pos.x + topPipe.size.width
    ) {
      topPipe.passed = true;
